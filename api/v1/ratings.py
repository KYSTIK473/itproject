...
async def like_film(
    request: Request,
    rating: float = Query(..., title="Оценка", description="Оценка фильма от 1 до 10", gt=0, le=10, alias="raiting"),  # валидация: 0
# < rating <= 10
    film_id: int = Query(..., title="ID фильма", description="ID фильма в базе данных", gt=0, alias="film_id")  # валидация: film_id
        # > 0
):
    try:
