...
### Фронтенд:
1.  Перейдите в директорию фронтенда (если репозиторий уже клонирован):
    ```bash
    cd itproject/frontend
    ```
2.  Убедитесь, что [бэкенд](#бэкенд) запущен и доступен на `http://localhost:8000`.

3.  Откройте главную страницу в браузере:
    ```bash
    # Для Linux:
...
4.  Для удобства разработки используйте Live Server:
    - Установите расширение для VS Code (например, `Live Server`)
    - Кликните правой кнопкой на `index.html` → `Open with Live Server`

⚠️ Важно: 
- Все запросы отправляются на `http://localhost:8000` (порт бэкенда)
- Для работы оценок и профиля требуется авторизация
- При изменении CSS/JS кэш браузера можно очистить сочетанием `Ctrl + F5`
